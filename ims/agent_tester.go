...
func init() {
	...
	agentHttpHost = jsoniter.Get(cfgData, "agent", "host").ToString()
	agentHttpPort = jsoniter.Get(cfgData, "agent", "ports", "http").ToInt()
}
...
func request(path string, body []byte) []byte {
	req, err := http.NewRequest("POST", fmt.Sprintf("http://%s:%d%s", agentHttpHost, agentHttpPort, path), bytes.NewReader(body))
...
func testBroker() {
	...
	if !assert.ObjectsAreEqual(true, jsoniter.Get(data, "brokers").Size() > 0) {
		logger.Fatal("Assert")
	}
}
...
func testGroup() {
	...
	body, _ = jsoniter.Marshal(params)
	data = request("/v1/group/is/member", body)
	if !assert.ObjectsAreEqual(true, jsoniter.Get(data, "ok").ToBool()) {
		logger.Fatal("Assert")
	}
	// List
