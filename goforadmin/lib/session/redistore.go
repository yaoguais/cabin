...
func (s *RediStore) Delete(r *http.Request, w http.ResponseWriter, session *sessions.Session) error {
	if err := s.Redis.Del(s.keyPrefix + session.ID).Err(); err != nil {
		return err
	}
...
func (s *RediStore) save(session *sessions.Session) error {
	...
	return s.Redis.Set(s.keyPrefix+session.ID, b, time.Duration(age)*time.Second).Err()
}

// load reads the session from redis.
// returns true if there is a sessoin data in DB
func (s *RediStore) load(session *sessions.Session) (bool, error) {
	data, err := s.Redis.Get(s.keyPrefix + session.ID).Result()
	if err != nil {
		return false, err
	}
	if data == "" {
...
func (s *RediStore) delete(session *sessions.Session) error {
	return s.Redis.Del(s.keyPrefix + session.ID).Err()
}
